...
// canLiftScopeTraversalExpression returns true if this variable access expression can be lifted. Any variable access expression
...
// proxied apply call.
func (g *generator) parseProxyApply(args []*model.ScopeTraversalExpression, then *model.AnonymousFunctionExpression) (*model.ScopeTraversalExpression, bool) {
	if len(args) != 1 {
		return nil, false
	}
...
func referencesCallbackParameter(expr model.Expression, parameters codegen.Set) bool {
	has := false
	_, diags := model.VisitExpression(expr, model.IdentityVisitor, func(expr model.Expression) (model.Expression, hcl.Diagnostics) {
...
func (g *generator) parseInterpolate(args []*model.ScopeTraversalExpression, then *model.AnonymousFunctionExpression) (model.Expression, bool) {
